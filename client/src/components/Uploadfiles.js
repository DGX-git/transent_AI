...
const AudioConverter = () => {
  ...
  // Transcription API call function
const transcribeAudio = async (file) => {
  const formData = new FormData();
  formData.append('file', file.file);

  try {
    // Call your local backend proxy instead of ngrok directly
    const response = await fetch('http://localhost:5000/transcribe/transcription', {
      method: 'POST',
      body: formData,
    });
  ...
  // Handle multiple file transcription
  const handleStartTranscription = async () => {
    ...
    try {
      // Get selected files
      const filesToTranscribe = uploadedFiles.filter((file) =>
        selectedFiles.includes(file.id)
      );

      console.log("Starting transcription for", filesToTranscribe.length, "files");

      // Transcribe all selected files (parallel processing)
      const transcriptionPromises = filesToTranscribe.map(file => 
        transcribeAudio(file)
      );

      const transcribedResults = await Promise.all(transcriptionPromises);

      // Navigate to results page with transcription data
      navigate("/transcription-results", {
        state: {
          transcribedFiles: transcribedResults,
        },
      });
    } catch (err) {
      setError("Failed to transcribe files. Please try again.");
